...
class EnhancedEmailSystem:
    ...
    def create_professional_html_template(self, validation_data, deadline_date):
        ...
        critical_amount = '[U8377]1,50,000'  # Sample amounts
        warning_amount = '[U8377]1,40,000'
            ...
            <div style="max-width: 800px; margin: 0 auto; background-color: #ffffff; border: 2px solid #e9ecef; border-radius: 10px; overflow: hidden;">
                ...
                <div style="padding: 30px; background-color: #ffffff;">
                    ...
                    <div style="background-color: #f8f9fa; border: 2px solid #dee2e6; border-radius: 8px; padding: 25px; margin-bottom: 25px;">
                        <h2 style="margin: 0 0 20px 0; color: #000000; font-size: 22px; font-weight: bold;">      [U128209] [U10004][U65039] KOENIG INVOICE VALIDATION REPORT</h2>
                        ...
                        <table style="width: 100%; border-collapse: collapse; margin-bottom: 20px;">
                            ...
                            <tr>
                                <td style="padding: 12px; border: 1px solid #dee2e6; background-color: #ffebee; color: #000000;">[U128680] CRITICAL ISSUES</td>
                            ...
                            <tr>
                                <td style="padding: 12px; border: 1px solid #dee2e6; background-color: #fff3e0; color: #000000;">[WARNING] WARNING ITEMS</td>
                            ...
                            <tr>
                                <td style="padding: 12px; border: 1px solid #dee2e6; background-color: #e8f5e8; color: #000000;">[OK] SUCCESSFULLY PROCESSED</td>
                    ...
                    <div style="background-color: #ffebee; border-left: 5px solid #d32f2f; padding: 20px; margin-bottom: 25px;">
                        <h3 style="margin: 0 0 15px 0; color: #000000; font-size: 18px;">[U128680] IMMEDIATE ACTION REQUIRED</h3>
                    ...
                    <div style="background-color: #ffffff; border: 2px solid #dee2e6; border-radius: 8px; padding: 25px; margin-bottom: 25px;">
                        <h3 style="margin: 0 0 20px 0; color: #000000; font-size: 18px;">[TARGET] REQUIRED ACTIONS</h3>
                    ...
                    <div style="background-color: #e3f2fd; border: 2px solid #2196f3; border-radius: 8px; padding: 20px; margin-bottom: 25px;">
                        <h3 style="margin: 0 0 15px 0; color: #000000; font-size: 18px;">[U128206] ATTACHMENTS INCLUDED</h3>
                    ...
                    <div style="background-color: #f5f5f5; border: 2px solid #9e9e9e; border-radius: 8px; padding: 20px;">
                        <h3 style="margin: 0 0 15px 0; color: #000000; font-size: 18px;">[U128222] FOR QUESTIONS OR SUPPORT</h3>
        ...
        try:
            ...
            with zipfile.ZipFile(zip_filename, 'w', zipfile.ZIP_DEFLATED) as zipf:
                ...
                if excel_files:
                    ...
                    logging.info(f"[STATS] Added Excel report to ZIP: {latest_excel}")
                ...
                logging.info(f"[U128193] ZIP created successfully: {zip_filename} ({invoice_count} invoice files)")
                return zip_filename
                
        except Exception as e:
            logging.error(f"[U10060] Error creating ZIP: {e}")
            return None
    ...
    def send_email_with_attachments(self, recipients, subject, html_body, zip_file):
        ...
        try:
            ...
            logging.info(f"[OK] Email sent successfully to: {', '.join(recipients)}")
            return True
            
        except Exception as e:
            logging.error(f"[U10060] Error sending email: {e}")
            return False
        finally:
            # Cleanup temporary ZIP file
            if zip_file and os.path.exists(zip_file):
                os.remove(zip_file)
                logging.info(f"[U128465][U65039] Temporary ZIP file cleaned up: {zip_file}")

def main():
    """Test the enhanced email system"""
    print("[U129514] Testing email system...")
    ...
    logging.info(f"[U128231] Email system initialized with {len(email_system.default_recipients)} recipients")
    
    # Find Excel report
    excel_files = glob.glob('invoice_validation_report_*.xlsx')
    if not excel_files:
        print("[STATS] No Excel reports found")
        return "No Excel reports found"
    
    latest_excel = max(excel_files, key=os.path.getctime)
    print(f"[STATS] Found Excel report: {os.path.basename(latest_excel)}")
    
    # Create ZIP with invoices
    zip_file = email_system.create_invoice_zip()
    if not zip_file:
        print("[U10060] Failed to create ZIP file")
    ...
    # Send email
    subject = f"[U128680] URGENT: Invoice Validation - Action Required by {deadline_date.strftime('%b %d, %Y')}"
    ...
    if success:
        ...
        print("[U10060] Email sending failed")
        return "Failed"

if __name__ == "__main__":
    logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')
    result = main()
    print(f"[U128231] Email result: {result}")
    print(f"[OK] Test result: {'Passed' if result == 'Success' else 'Failed'}")
